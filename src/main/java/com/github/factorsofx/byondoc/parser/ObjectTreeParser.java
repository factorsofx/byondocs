 ...
 * <strike>The eventual goal is to try to make it multithreaded</strike> That <strike>probably</strike>definitely won't happen.
 * <strike><em>STILL WIP - DO NOT USE</em></strike> fuck it it's close enough
 */
public class ObjectTreeParser
{
    ...
    private static final char COMMENT_DELIMITER_TOKEN = '\u001E'; // This shouldn't show up in the code at all
    private static final Pattern PROCESSED_COMMENT_MATCHER = Pattern.compile("\u001E(.*)\u001E", Pattern.DOTALL);
    ...
    public Future<ObjectTree> parse()
    {
    ...
    private void subParse(ObjectTree tree, BufferedReader reader, Path currentFile) throws IOException
    {
        ...
        while((currentLine = reader.readLine()) != null)
        {
            ...
            for(int i = 0; i < currentLine.length(); i++)
            {
                ...
                if(inComment)
                {
                    ...
                    {
                        inComment = false;
                        lineBuffer.append(COMMENT_DELIMITER_TOKEN);
                        i++;
                    }
                    else
                    {
        ...
        for(String line : lines)
        {
            ...
            Matcher commentMatcher = PROCESSED_COMMENT_MATCHER.matcher(line);
            ...
            if(!(lowestProcDepth >= 0 && indentDepth >= lowestProcDepth))
            {
                ...
                EnumSet<VarFlag> varFlags = EnumSet.noneOf(VarFlag.class);
                ...
                for(CharSequence pathSequence : splitLine)
                {
                    ...
                    if(pathSection.isEmpty() || pathSection.equals("const") || pathSection.equals("static") || pathSection.equals("global") || pathSection.equals("tmp"))
                    {
                        ...
                        {
                            varFlags.add(VarFlag.valueOf(pathSection.toUpperCase()));
                        }
                        continue;
                    }
                ...
                if(matcher.find())
                {
                    ...
                    Var var = new Var(varName, varVal, currentFile.toString(), varFlags);
                    tree.getOrCreateObjectNode(fullPathBuilder.toString().trim()).addVar(var);
                    if(!isBlank(lastComment))
                    {
                        var.setDoc(lastComment);
                        lastComment = "";
                    }
                }
                else
                {
                    if(!(isBlank(lastComment) || isBlank(line)))
                    {
                        ObjectNode node = tree.getOrCreateObjectNode(fullPathBuilder.toString().trim());
                        node.setDoc(lastComment);
                        lastComment = "";
                    }
                }
            }
